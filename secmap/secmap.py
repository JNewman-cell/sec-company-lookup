...
def _load_data_to_memory(data: Dict[str, Any]):
    ...
    logger.info(f"Found {multi_cik_count} CIKs with multiple companies, {multi_name_count} names with multiple companies")
...
def _ensure_data_loaded():
    ...
    if not _memory_cache or is_cache_expired(_last_update):
        ...
        if cached_data:
            ...
            if not update_data():
                raise RuntimeError("Unable to load SEC company data. Please check your internet connection.")
...
def search_companies(query: str, limit: int = 10, fuzzy: bool = True) -> List[Dict[str, Any]]:
    """
    ...
    if len(results) < limit:
        try:
            ...
            logger.warning(f"Database search failed, falling back to memory search: {e}")
            ...
            for mem_result in memory_results:
                if len(results) >= limit:
                    break
                is_duplicate = any(
                    r['cik'] == mem_result['cik'] and r['ticker'] == mem_result['ticker'] 
                    for r in results
                )
                if not is_duplicate:
...
def _search_companies_memory(query: str, limit: int = 10, fuzzy: bool = True) -> List[Dict[str, Any]]:
...
def get_companies_by_ciks(ciks: List[Union[int, str]]) -> Dict[Union[int, str], List[Dict[str, Any]]]:
    """
    ...
    try:
        ...
        logger.warning(f"Database batch lookup failed, falling back to individual lookups: {e}")
        # Fallback to individual lookups
        return {cik: get_company_by_cik(cik) for cik in ciks}


def get_companies_by_company_names(company_names: List[str], fuzzy: bool = True) -> Dict[str, List[Dict[str, Any]]]:
    """
    ...
    for company_name in company_names:
        ...
        if company_ids:
            ...
            results[company_name] = [_memory_cache['companies'][company_id] for company_id in company_ids]
            if not fuzzy:
    ...
    if remaining_names:
        try:
            ...
            logger.warning(f"Database batch company name lookup failed, falling back to memory search: {e}")
            ...
            for company_name in remaining_names:
                ...
                existing_pairs = {(c['cik'], c['ticker']) for c in results[company_name]}
                
                for fallback_result in fallback_results:
                    if (fallback_result['cik'], fallback_result['ticker']) not in existing_pairs:
                        results[company_name].append(fallback_result)
    
    return results


def search_companies_by_company_name(company_name_query: str, limit: int = 10, fuzzy: bool = True) -> List[Dict[str, Any]]:
    """
    ...
    if len(results) < limit:
        try:
            db_results = search_companies_by_company_name_db(query_stripped, limit, fuzzy)
            ...
            logger.warning(f"Database company name search failed, falling back to memory search: {e}")
            ...
            for fallback_result in fallback_results:
                if len(results) >= limit:
                    break
                is_duplicate = any(
                    r['cik'] == fallback_result['cik'] and r['ticker'] == fallback_result['ticker'] 
                    for r in results
                )
                if not is_duplicate:
...
def get_companies_by_sector_search(sector_keywords: List[str], limit: int = 50) -> List[Dict[str, Any]]:
    """
...
def get_cache_info() -> Dict[str, Any]:
    ...
    return {
        ...
        'cache_age_hours': (time.time() - _last_update) / 3600 if _last_update else None,
