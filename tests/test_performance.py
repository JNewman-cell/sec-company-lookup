...
class TestPerformance:
    ...
    def test_performance_targets(self, mock_ensure):
        ...
        assert (
            avg_time_ms < 10.0
        ), f"Cached lookup took {avg_time_ms:.3f}ms, expected < 10ms"
    ...
    def test_memory_efficiency(self, mock_ensure):
        ...
        try:
            ...
            assert (
                memory_growth < 10
            ), f"Memory grew by {memory_growth:.1f}MB, expected < 10MB"
        except ImportError:
    ...
    def test_batch_processing_efficiency(self, mock_ensure):
        ...
        ), f"Batch processing {efficiency_ratio:.1f}x speed, expected >0.01x"
        assert len(batch_results) == len(
            set(ciks)
        ), "Batch processing should return results for all unique CIKs"
    ...
    def test_cache_warming_performance(self):
        ...
        assert (
            cache_warm_ms < 10
        ), f"Cache warming took {cache_warm_ms:.3f}ms, expected < 10ms"
