...
class TestSecmapCore:
    ...
    def setup_method(self):
        ...
        self.download_patcher = patch('secmap.utils.utils.download_sec_data', return_value=SAMPLE_SEC_DATA)
        self.download_mock = self.download_patcher.start()
        
        self.load_cache_patcher = patch('secmap.utils.utils.load_from_cache', return_value=(SAMPLE_SEC_DATA, time.time()))
    ...
    def test_search_companies_by_company_name(self):
        ...
        with patch('secmap.secmap.search_companies_by_company_name_db', side_effect=sqlite3.Error("No DB")):
        ...
        with patch('secmap.secmap.search_companies_by_company_name_db', side_effect=sqlite3.Error("No DB")):
            results = search_companies_by_company_name("NonExistent Company", fuzzy=False)
            assert len(results) == 0
    ...
    def test_get_companies_by_sector_search(self):
        ...
        # Mock database function to avoid database dependency
        with patch('secmap.db.db.get_companies_by_sector_search_db', side_effect=Exception("No DB")):
...
class TestEdgeCases:
    ...
    def test_invalid_cik_formats(self):
        ...
        invalid_ciks = ["abc", "123abc", "12.34", "-123", "", "   ", "invalid", "123.456"]
        
        for cik in invalid_ciks:
            result = get_company_by_cik(cik)
            assert result == [], f"Expected empty list for invalid CIK: {cik}, got: {result}"
    ...
    def test_memory_cache_corruption_resistance(self):
        ...
        with patch('secmap.secmap._ensure_data_loaded', side_effect=RuntimeError("Cache corrupted")):
...
class TestIntegrationAndPerformance:
    ...
    def test_database_fallback_integration(self):
        ...
        with patch('secmap.db.db.search_companies_db', side_effect=sqlite3.Error("DB Error")):
    ...
    def test_cache_info_comprehensive(self):
        ...
        expected_keys = [
            'companies_cached', 'last_update', 'cache_age_hours', 'cache_expired',
            'ticker_cache_info', 'cik_cache_info', 'cache_dir', 'data_file_exists',
    ...
    def test_concurrent_access_simulation(self):
        ...
        aapl_results = [r for r in results if isinstance(r, dict) and r.get('ticker') == 'AAPL']
